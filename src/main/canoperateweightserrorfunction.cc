...
ErrorFunctionType CanOperateWeightsErrorFunction::GetType() const {
    return ErrorFunctionTypeCanOperateWeights;
}

MWVector<Matrix<Data> > CanOperateWeightsErrorFunction::GetGlobalWeight(
    MWVector<MWVector<Matrix<Data> > > &weight_vector_vector_matrix) const {
    MWVector<Matrix<Data> > ret = GetGlobalWeightStructure(weight_vector_vector_matrix);
    return AssignGlobalWeight(weight_vector_vector_matrix, ret);
}
