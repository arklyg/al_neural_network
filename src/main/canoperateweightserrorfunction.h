...
class CanOperateWeightsErrorFunction : public ErrorFunction {
  public:
    ...
    virtual ErrorFunctionType GetType() const;

    virtual Vector<Matrix<Data> > &AssignGlobalWeight(
        Vector<Vector<Matrix<Data> > > &weight_vector_vector_matrix,
        Vector<Matrix<Data> > &ret) const = 0;
    virtual Vector<Matrix<Data> > &AssignGlobalWeightMultiThread(
        Vector<Vector<Matrix<Data> > > &weight_vector_vector_matrix,
        Vector<Matrix<Data> > &ret) const = 0;

    virtual MWVector<Matrix<Data> > GetGlobalWeightStructure(
        const MWVector<MWVector<Matrix<Data> > > &weight_vector_vector_matrix) const = 0;

    MWVector<Matrix<Data> > GetGlobalWeight(
